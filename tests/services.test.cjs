...
const content = require('../miniprogram/services/content-service');
...
test('sequence advances on ended and destroys context',async()=>{const f=fixture(),a=createAudioService(()=>f.create(),asset);const result=a.play(['a','b']);assert.deepEqual(f.contexts[0].played,['/a.mp3']);f.contexts[0].ended();assert.deepEqual(f.contexts[0].played,['/a.mp3','/b.mp3']);f.contexts[0].ended();assert.equal(await result,true);assert.ok(f.contexts[0].destroyed);});
