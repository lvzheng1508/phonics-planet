...
Page({
 ...
 onLoad({id}){const item=content.phoneme(id);if(!item)return;this.setData({item,spellings:item.commonSpellings.join(' · '),audioIds:[item.audioId],contrasts:item.contrast.map(content.phoneme).filter(Boolean).map(p=>({...p,url:route('phoneme-detail',{id:p.id})})),words:wordCards(content.words().filter(w=>content.detail(w.id).phonemes.some(p=>p.id===id)).slice(0,8))});},
