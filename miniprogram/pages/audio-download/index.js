...
Page({
  data:{status:'idle',heading:labels.idle,progress:0,count:0,total:0,size:'',busy:false,ready:false,error:'',checking:true,resetting:false},
  onLoad(){this.alive=true;this.off=bundle.subscribe(s=>{if(this.alive)this.setData({...s,heading:this.data.resetting?'正在清除本地音频':labels[s.status],busy:this.data.resetting||['downloading','verifying','extracting'].includes(s.status),size:(s.bytes/1000000).toFixed(2)});});},
  async onShow(){
  ...
  async redownload(){
    ...
    this.setData({resetting:true,busy:true,error:'',heading:'正在清除本地音频',progress:0});
    try{
      audio.stop();
      await resources.clear();
    }catch(error){
      if(this.alive)this.setData({resetting:false,busy:false,status:'error',heading:'暂时没有清除成功',error:'本地音频未能清除，请重试'});
      return;
    }
    if(!this.alive)return;
    this.setData({resetting:false,ready:false,count:0});
    try{await bundle.install();}catch(_){}
  },
