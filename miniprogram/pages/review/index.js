...
Page({
 ...
 onShow(){this.off=audio.subscribe(state=>{const playing=state.owner==='review-whole'&&this.data.current&&state.audioId===this.data.current.audioId&&state.status==='playing';this.setData({playing:!!playing,ipaTokens:this.data.current?progressTokens(this.data.current.ipaTokens,playing?state.progress:0,playing?state.currentTime:0):[]});});},
